package starbucks;import java.util.ArrayList;
...
public class KeyPad implements ITouchEventHandler, IDisplayComponent, IKeyPadSubject
{
     ...
     */
    private static void setcountPinDigits(int value)
    {
      countPinDigits = value;
    }
     ...
     *@param value [description]
     */
    private static void setlastKey(String value)
    {
      lastKey  = value;
    }
    ...
    public void touch(int x, int y) { 
        if ( y > 4 )
        {
            ...
            setlastKey(getKey( x, y )) ;
            if ( x==3 && y==8   )
            {
                setcountPinDigits(countPinDigits - 1) ;
            }
            else if ( y < 8 || (x==2 && y==8))
            {
                setcountPinDigits(countPinDigits + 1) ;
    ...
    */
     ...
     */
    /**
     * Set countPinDigits and lastkey to 0 if the pin is invalid
     * @param c Pin count
     */
    public void invalidpin(int c)
    {
        setcountPinDigits(c);
        setlastKey("");
    }

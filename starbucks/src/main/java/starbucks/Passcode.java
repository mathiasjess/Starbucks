package starbucks;/** Passcode Screen Component */
public class Passcode implements ITouchEventHandler, IDisplayComponent, IKeyPadObserver
{
    ITouchEventHandler nextHandler ;
    private static int count ;
    private static boolean invalidpin  ;

    /**
     * Constructor for objects of class Passcode
     */
    public Passcode()
    {
        setcount(0);
        setinvalidpin(false);
    }

    /**
     * Constructor for objects of class AddCardCode
     *@param value [description]
     */
    private static void setcount(int value)
    {
      count = value;
    }
    /**
     * Constructor for objects of class AddCardCode
     *@param value [description]
     */
    private static void setinvalidpin(boolean value)
    {
      invalidpin = value;
    }
    /**
     * Touch Event Ignored by Passcode
     * @param x Touch X
     * @param y Touch Y
     */
    public void touch(int x, int y) 
    {
        if ( y==2 )
        {
            System.err.println( "Passcode Touched at (" + x + ", " + y + ")" ) ; 
        }
        else
        {
            if ( nextHandler != null )
                nextHandler.touch(x,y) ;
        }
    }
    
    /**
     * Set Next Touch Handler
     * @param next Touch Event Handler
     */
    public void setNext( ITouchEventHandler next) 
    { 
        nextHandler = next ;
    }
    
    
    /**
     * Display "Echo Feedback" on Pins enterred so far
     * @return Passcode String for Display
     */
    public String display() 
    {
        String value = "" ;
        if(invalidpin == true)
        {   
            setinvalidpin(false);
            value+= "  Invalid Pin\n\n ";
        }
        switch ( count )
        {
            case 0: value+= " [_][_][_][_]" ; break ;
            case 1: value+= " [*][_][_][_]" ; break ;
            case 2: value+= " [*][*][_][_]" ; break ;
            case 3: value+= " [*][*][*][_]" ; break ;
            case 4: value+= " [*][*][*][*]" ; break ;
            default: value+= "";
        }
         return value  ;
    }
    
    /**
     * Add Sub Component (Not used)
     * @param c Sub Component to Add
     */
    public void addSubComponent( IDisplayComponent c ) 
    {
        
    }   
    
    /**
     * Key Event Update
     * @param c   Count of Keys So Far
     * @param key Last key Pressed
     */
    public void keyEventUpdate( int c, String key ) 
    {
        System.err.println( "Key: " + key ) ;
        setcount(c) ;
    }
    
    /**
     * Key Event Update
     * @param c   Count of Keys So Far
     * @param key Last key Pressed
     */
    public void invalidpin( int c, String key ) 
    {
        System.err.println( "Key: " + key ) ;
        setcount(c) ;
        setinvalidpin(true);
    }
}
